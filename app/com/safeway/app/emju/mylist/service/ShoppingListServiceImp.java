...
public class ShoppingListServiceImp implements ShoppingListService {
	...
	public ShoppingListServiceImp(ShoppingListDAO shoppingListDAO, OfferStatusService offerStatusService,
			...
			ClubPriceDAO clubPriceDao, MiscEntityCache miscEntityCache, StoreCache storeCache, 
			EmailBroker emailBroker, StoreDAO storeDAO) {
	...
	public List<ShoppingListVO> getShoppingList(ShoppingListVO shoppingListVO) throws ApplicationException {
		...
		List<ShoppingListItemVO> deletedItems = 
				fromTime == null ? null : new ArrayList<ShoppingListItemVO>();
		...
		try {
			...
			Map<String, Map<String, ShoppingListItem>> shoppingListItemsMap = 
					new HashMap<String, Map<String, ShoppingListItem>>();
			...
			if(ValidationHelper.isEmpty(shoppingListItems)) {
				...
				LOGGER.warn("Shopping List NOT exist for CUSTOMER_GUID: "
						+ shoppingListVO.getHeaderVO().getSwycustguid());
	...
	public Integer getShoppingListCount(ShoppingListVO shoppingListVO, String listName)
	...
	public void sendShoppingListMail(MailListVO mailListVO, HeaderVO headerVO)
			throws ApplicationException {
        ...
        if (ValidationHelper.isNullOrEmptyArray(toEmails)
            || ValidationHelper.isNullOrEmptyArray(itemIds)) {
        ...
        if (ValidationHelper.isNonEmpty(integratedShoppingList)) {
        	...
        	if (ValidationHelper.isNonEmpty(shoppingList.getItems())) {
                    ...
                    slNotification.setGroups(orderShoppingListForEmail(items,
                        mailListVO));
	...
	private void setPreferredStoreInfo(HeaderVO headerVO, String selectedStoreId) {
        ...
        try {
            ...
            preferredStore = storeDAO.findStoreInfo(storeId, 
            		Long.valueOf(headerVO.getSwyhouseholdid()), headerVO.getPostalcode());                                         
	...
	private void processShoppingList(List<ShoppingListItem> shoppingListItems,
			...
			List<String> itemIdsList, boolean hasItemIdFilter, boolean[] versionValues, 
			Date fromTime, List<ShoppingListItemVO> deletedItems, Integer storeId) {
		...
		if (ValidationHelper.isNonEmpty(shoppingListItems)) {
			...
			for (ShoppingListItem shoppingListItem : shoppingListItems) {
				...
				itemTypeCd = itemTypeCd.equals("MF") ||  
						itemTypeCd.equals("SC") ?
				...
				if(fromTime == null || fromTime.before(shoppingListItem.getLastUpdTs())) {
					...
					if (itemTypeCd.equalsIgnoreCase(Constants.ItemTypeCode.COUPON_ITEM.toString())
						...
						mapKey = itemId;
						if (ValidationHelper.isEmpty(redeemedOfferList) || !redeemedOfferList.contains(Long.valueOf(itemId))) {
							canBeProcess = true;
						}
	...
	private void setItemDetails(Map<String, Map<String, ShoppingListItem>> shoppingListItemsMap, 
			ShoppingListVO shoppingListVO, List<ShoppingListItemVO> newSLItemVoSet,
			String details) throws ApplicationException {
...
//		String itemType = null;
			...
			if (ValidationHelper.isNonEmpty(itemMap)) {
				if(itemType.equals(ItemTypeCode.STANDARD_PRODUCT_ITEM.toString())) {
					...
					if(ValidationHelper.isNonEmpty(details) && details.equals(Constants.YES)){
						matchedOffers = matchOfferService
							.getRelatedOffers(upcItemMap, shoppingListVO.getHeaderVO());
					}
					...
					newSLItemVoSet.addAll(itemDetaislService.setItemDetails(itemType, itemMap, 
							shoppingListVO, matchedOffers));
		...
		for(Entry<String, Promise<Map<Long, Object>>> entry : offerDetails.entrySet()) {
			...
			if(ValidationHelper.isNonEmpty(itemDetail)) {
				...
				newSLItemVoSet.addAll(itemDetaislService.getItemDetails(entry.getKey(), itemDetail, itemMap, shoppingListVO));
			}
		}
	...
	private EmailItemGroup[] orderShoppingListForEmail(
	        final List<ShoppingListItemVO> itemsVOS, final MailListVO criteria) {
        	...
        	for (String itemId : itemIds) {
        		...
        		for (ShoppingListItemVO currentItem : itemVOSList) {
        			...
        			if (itemId.equals(currentItem.getId())) {
                        ...
                        shoppingListItemDetail.setCategoryName(currentItem
                            .getCategoryName());

                        DateFormat formatter = new SimpleDateFormat(
                        "MM/dd/yyyy");
                        DateFormat fromGetFormatter = new SimpleDateFormat(
                        ...
                        if (null != currentItem.getImage()) {
                            ...
                            if (Constants.J4U.contains(currentItem
                                .getItemType())) {
                                imageUrl.append(J4U_IMAGE_URL);
                            }
                            imageUrl.append(currentItem.getImage());
                            shoppingListItemDetail.setImage(imageUrl
                                .toString());
                        }
                        ...
                        String itemDescription = 
                        		currentItem.getDescription() == null ? null: parseDescription(currentItem);
                        ...
                        if(currentItem.getItemType().equalsIgnoreCase(Constants.ItemTypeCode.STANDARD_PRODUCT_ITEM.toString())
                                || currentItem.getItemType().equalsIgnoreCase(Constants.ItemTypeCode.CLUB_SPECIAL_ITEM.toString())){
                        	
                        	LOGGER.debug("Image Type: " + currentItem.getImage());
                            //Validate the image url is not exist then it default to ycs url
                            if(null == currentItem.getImage()){
                                StringBuffer imageUrl = new StringBuffer();
                                
                                String imageURL = YCS_IMAGE_URL+ currentItem.getReferenceId() +
                                		YCS_IMAGE_EXT;
                            ...
                            if(currentItem.getItemType().equalsIgnoreCase(Constants.ItemTypeCode.CLUB_SPECIAL_ITEM.toString())) {
                                double promoPrice = 0;

                                if (currentItem.getSavingsValue() != null) {
                                    //promoPrice = ((BigDecimal) Long.valueOf(currentItem.getSavingsValue())).doubleValue();
                                ...
                                if (priceMethodType != null
                                    && priceMethodType
                                    .equalsIgnoreCase(Constants.PRICE_METHOD_TYPE_BOGO)) {
                                    if (priceMethodSubType != null
                                        && priceMethodSubType
                                ...
                                shoppingListItemDetail.setType(types.get(Constants.ItemTypeCode.CLUB_SPECIAL_ITEM.toString()));
                            ...
                            shoppingListItemDetail.setSavings(savingsSuffix.get(currentItem.getItemType())
                                + currentItem.getSavingsValue());
                        }
